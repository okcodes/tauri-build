...
export const uploadTextAsAsset = async ({ releaseId, githubToken, name, text }: UploadTextParams): Promise<void> => {
  try {
    console.log('Will upload text as asset', { releaseId, name })
    const octokit = new Octokit({ auth: githubToken })
    const release = await octokit.repos.getRelease({ release_id: releaseId, owner: 'okcodes', repo: 'tauri-release-action' })
    const data = Buffer.from(text) // TODO: Maybe I don't need to create a buffer and I can pass data directly.
